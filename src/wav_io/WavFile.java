...
public class WavFile
{
	...
	public static WavFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		if (file.length() != chunkSize+8) throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + file.length() + ")");
	...
	private long readSample() throws IOException, WavFileException
	{
		...
		for (int b=0 ; b<bytesPerSample ; b++)
		{
			...
			//System.out.println("buffer: "+ buffer[bufferPointer]+"  "+bufferPointer); 
	...
	{
		out.printf("The File: %s\n", file);
		out.printf("IO State: %s\n\n", ioState);
		out.printf("Headerausgabe\n");
		out.printf("Channels: %d\n", numChannels);
		out.printf("Frames: %d\n", numFrames);
		out.printf("Sample Rate: %d\n", sampleRate);
		out.printf("Valid Bits: %d\n", validBits);
		out.printf("Bytes per sample: %d\n\n\n", bytesPerSample);
	}
	
	public static void write_wav(String out_file, int numChannels, long numFrames, int validBits, long sampleRate, short sound[]) throws IOException, WavFileException
	{
		WavFile writeWavFile = newWavFile(new File(out_file), numChannels, numFrames, validBits, sampleRate);
		for (int i=0; i < numFrames*numChannels;i++)
			writeWavFile.writeSample(sound[i]);
		writeWavFile.display();
		writeWavFile.close();
	}

	public static WavFile read_wav(String in_file) throws IOException, WavFileException
	{
		WavFile readWavFile = openWavFile(new File(in_file));
		int validbits = readWavFile.validBits;
		if(validbits != 16) throw new WavFileException("Bitanzahl von (" + validbits + ") wird nicht unterst�tzt");
		int samples=readWavFile.numChannels*(int)readWavFile.numFrames;
		readWavFile.display();
					
		//allocated buffer for complete audio data
		readWavFile.sound = new short[samples]; //allocate sample buffer of 16bit values
		for (int i=0; i < samples;i++)
			readWavFile.sound[i]=(short)readWavFile.readSample();
		readWavFile.close();
		return readWavFile;
	}
}
